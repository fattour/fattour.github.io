...
(function(global, factory) {
    ...
    $.fn.fullpage = function(options) {
        ...
        */
        ...
        var previousDestLeft = 0;
        /**
        * Returns the destination Y position based on the scrolling direction and
        * the height of the section.
        */
        function getDestinationLeft(element){
            var elemPosition = element.position();

            //top of the desination will be at the top of the viewport
            var position = elemPosition.left;
            var isScrollingRight =  elemPosition.left > previousDestLeft;
            var sectionBottom = position - $(window).width() + element.outerWidth();
            var bigSectionsDestination = options.bigSectionsDestination;

            //is the destination element bigger than the viewport?
            if(element.outerWidth() > $(window).width()){
                //scrolling up?
                if(!isScrollingRight && !bigSectionsDestination || bigSectionsDestination === 'bottom' ){
                    position = sectionBottom;
                }
            }

            //sections equal or smaller than the viewport height && scrolling down? ||  is resizing and its in the last section
            else if(sectionBottom || (isResizing && element.is(':last-child')) ){
                //The bottom of the destination will be at the bottom of the viewport
                position = sectionBottom;
            }
        ...
        */
        function scrollPage(element, callback, isMovementUp){
            ...
            var dleft = getDestinationLeft(element);
            var slideAnchorLink;
            var slideIndex;
        ...
        */
        function performMovement(v){
            ...
            if (options.css3 && options.autoScrolling && !options.scrollBar) {
                ...
                var translate3d = 'translate3d(0px, -' + Math.round(v.dtop) + 'px, 0px)';
                if(options.scrollY){
                    translate3d = 'translate3d( -' + Math.round(v.dleft) + 'px, 0px, 0px)';
                }
